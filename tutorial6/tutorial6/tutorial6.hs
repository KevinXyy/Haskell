...
isBloodOrange (Orange str _) | str `elem` ["Torocco", "Moro", "Sanguinello"] = True
                              | otherwise = False
isBloodOrange (Apple str _) = False

-- 2.
bloodOrangeSegments :: [Fruit] -> Int
bloodOrangeSegments xs = sum [i | (Orange str i) <- xs, isBloodOrange (Orange str i)]

-- 3.
worms :: [Fruit] -> Int
worms xs = length [ t | (Apple str t) <- xs, t == True]
...
tautology wf = and [eval x wf | x <-envs (atoms wf)]

prop_taut1 :: Eq a => Wff a -> Bool
prop_taut1 wf = tautology wf || satisfiable (Not wf)

prop_taut2 :: Eq a => Wff a -> Bool
prop_taut2 wf = not (satisfiable wf) || not (tautology (Not wf)) 
...
equivalent w1 w2 = and [eval e w1 == eval e w2 | e <- envs (nub (atoms w1 ++ atoms w2)) ]   
-- 8.
...
toNNF (p :|: q)        = toNNF p :|: toNNF q
toNNF x                = x           -- (V a), Not (V a), T, F
-- check if result of toNNF is in neg. normal form
prop_NNF1 :: Eq a => Wff a -> Bool
